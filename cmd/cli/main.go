...
func main() {
	...
	// Large models (30B+) may need more time to respond
...
IMPORTANT: After gathering enough information (2-3 searches), provide a final summary. Do not search endlessly.`,
	)
	agentInstance.WithMaxIterations(10)

	task := agent.NewTask(
		agent.TaskID("task-analyze-"+string(evt.IndexID)),
		"analyze-index",
		fmt.Sprintf("Analyze the file index with ID '%s' containing %d files. Use the search_index tool to find interesting files, then provide a brief summary of the project structure. Limit yourself to 3-4 searches maximum.", evt.IndexID, evt.FileCount),
	)
